...
class MelDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not self.fine_tuning:
            ...
            mel = np.load(
                filename.replace(self.base_wavs_path.rstrip("/"), self.base_mels_path.rstrip("/")).replace(
                    ".wav", ".npy"
                )
            )
        ...
        if mel_loss.size(2) != mel.size(2):
            mel_loss = mel_loss[:, :, : mel.size(2)]  # Adjust if mel loss is a bit longer than the loaded spec
